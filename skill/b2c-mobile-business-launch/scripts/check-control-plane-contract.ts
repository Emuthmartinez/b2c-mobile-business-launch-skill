...
if (loaded.state && isRecord(loaded.state)) {
  ...
  if (!controlPlane) {
    ...
    const futurePanels = new Set(asArray(controlPlane.futurePanels).map((panel) => asString(panel)).filter((panel): panel is string => Boolean(panel)));
    ...
    for (const [panelId, minimumStatus] of requiredPanels) {
      ...
      if (panelId === "design-room" && status !== "active") {
        ...
        issues.push(issue("error", `control_plane.${panelId}.not_needed`, `${panelId} cannot be marked not_needed in the skill template. It must be planned until implemented or deliberately blocked.`, args.statePath));
      } else if (!status) {
      ...
      const renderedArtifacts = asArray(panel.renderedArtifacts).map((entry) => asString(entry)).filter(Boolean);
      ...
      if (renderedArtifacts.length === 0) {
        issues.push(issue("error", `control_plane.${panelId}.rendered_artifacts_missing`, `${panelId} must name at least one rendered panel/artifact.`, args.statePath));
      }
    ...
    for (const panelId of requiredPanels.keys()) {
      if (futurePanels.has(panelId)) {
        issues.push(issue("error", `control_plane.${panelId}.future_only`, `${panelId} must be in controlPlane.panels, not only futurePanels.`, args.statePath));
      }
    }
