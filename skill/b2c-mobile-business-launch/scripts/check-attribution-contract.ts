...
function resolveEvidence(evidencePath: string): string {
  return path.isAbsolute(evidencePath)
    ? evidencePath
    : path.join(args.root, evidencePath);
}
  ...
  if (laneStatus !== "not_needed" && laneStatus !== "not_started") {
    ...
    for (const field of requiredBooleans) {
      if (asBoolean(getPath(state, `${base}.${field}`)) !== true) {
        issues.push(issue(booleanSeverity, `attribution.${field}.incomplete`, `${base}.${field} must be true before attribution is launch-ready.`, "PROJECT_STATE.yaml"));
      }
    }
