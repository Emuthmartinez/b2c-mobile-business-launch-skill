...
Motion is a tokenized, cross-platform contract. Values live once in `state/theme.tokens.json` under `motion.*` (`durationFast`, `durationBase`, `durationSlow`, `reducedMotionDuration`, `easing`, plus the landing-surface scale `durationReveal`, `durationCinematic`, `easingEmphasis`, `easingSpring`, `stagger`) and promote to `design-system/tokens.css` (`--motion-*` variables) and `DesignTokens.Motion` in `DesignTokens.swift`. `promote-design-tokens.ts` writes them and `check-token-promotion.ts` gates them.
...
- **Premium landing motion has a craft layer.** For motion-rich landing pages and funnels (the "motionsites-grade" bar), load [`landing-motion-craft.md`](landing-motion-craft.md): the two-lane model (Remotion-baked hero/section video + `motion/react` live choreography sharing the `--motion-*` scale), the section catalog, and the progressive-enhancement contract. Seed the buildless section library from `templates/landing-page/`.
