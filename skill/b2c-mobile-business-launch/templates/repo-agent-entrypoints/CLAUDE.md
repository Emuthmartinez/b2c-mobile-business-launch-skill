...
- Runtime split: Claude owns the pre-build stages through the spec (research, 11-star, design, growth, analytics, spec/handoff); Codex owns the core app build. For long-running, massively parallel, or adversarial pre-build stages use a Dynamic Workflow (`ultracode`, `/effort ultracode`, or `/deep-research`) — set a token budget, pair loops with `/goal`, quarantine untrusted reviews/social/scraped input, and keep producer and verifier agents separate. Do not spend a Claude workflow on the build Codex owns; fall back to subagents and record the reason when workflows are unavailable. Record the split in `ORCHESTRATION.md` and `PROJECT_STATE.yaml`.
